...
class CustomerFlowEdgeCaseResource(SyncAPIResource):
    ...
    def add(
        ...
        *,
        ...
        A scripted flow's variants are owned by the
        path engine, one per path through the graph, so they are created by editing the
        graph through PUT /v1/customer-flow/{flowId}/graph rather than here. Leave
        ...
        if not flow_id:
            raise ValueError(f"Expected a non-empty value for `flow_id` but received {flow_id!r}")
        return self._post(
            f"/v1/customer-flow/{flow_id}/edge-case",
            body=maybe_transform(
                {
                    "title": title,
                    "environment_id": environment_id,
    ...
    def promote(
        ...
        *,
        ...
        if not flow_id:
            raise ValueError(f"Expected a non-empty value for `flow_id` but received {flow_id!r}")
        return self._post(
            f"/v1/customer-flow/{flow_id}/edge-case/{edge_case_id}/promote",
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
